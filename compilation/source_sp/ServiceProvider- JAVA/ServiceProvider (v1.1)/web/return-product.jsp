...
<%
    ...
    try {
        ...
        String returned = (now.get(Calendar.YEAR)+"-"+monthStr+"-"+day+" "+hour+":"+minute+":"+second);
        ...
        ps = con.prepareStatement("UPDATE customer_service NATURAL JOIN service "
                + "SET returned='"+returned+"', status='success' WHERE spID='"+account.getUsername()+"'"
                        + "AND customerID='"+customer+"' AND serviceID='"+serviceID+"'");
        ps.executeUpdate();
        response.sendRedirect("dispatched-products.jsp");
